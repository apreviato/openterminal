...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/openterminal"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "openterminal")
      default:
        return "/etc/openterminal"
    }
  }
  ...
  export const state = Instance.state(async () => {
    ...
    // .openterminal directory config overrides (project and global) config sources.
    ...
    for (const dir of unique(directories)) {
      if (dir.endsWith(".openterminal") || dir === Flag.OPENCODE_CONFIG_DIR) {
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.openterminal/command/", "/.openterminal/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.openterminal/agent/", "/.openterminal/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: openterminal.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for openterminal serve and web commands"),
      command: z
      ...
      compaction: z
        .object({
          ...
          prune_minimum: z
            ...
            .min(0)
            .optional()
            .describe("Minimum tokens that must be prunable before a prune pass runs (default: 20000)."),
          prune_protect: z
            .number()
            .int()
            .min(0)
            .optional()
            .describe("Tokens of recent tool output to keep when pruning (default: 40000)."),
        })
        .optional(),
      experimental: z
        .object({
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    const normalized = (() => {
      ...
      delete copy.tui
      log.warn("tui keys in openterminal config are deprecated; move them to tui.json", { path: source })
      return copy
    })()
