...
const TIPS = [
  ...
  "Run {highlight}/models{/highlight} or {highlight}Ctrl+X M{/highlight} to see and switch between available Ollama models",
  themeTip,
  ...
  "Place TUI settings in {highlight}~/.config/openterminal/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default Ollama model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local MCP servers in the {highlight}mcp{/highlight} config section",
  "OpenTerminal auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.openterminal/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.openterminal/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "OpenTerminal auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "OpenTerminal uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.openterminal/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.openterminal/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenTerminal from reading sensitive files",
  "Use {highlight}openterminal run{/highlight} for non-interactive scripting",
  "Use {highlight}openterminal --continue{/highlight} to resume the last session",
  "Use {highlight}openterminal run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}openterminal serve{/highlight} for headless API access to OpenTerminal",
  "Use {highlight}openterminal run --attach{/highlight} to connect to a running server",
  "Run {highlight}openterminal agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.openterminal/themes/{/highlight} directory",
  ...
  "Run {highlight}openterminal debug config{/highlight} to troubleshoot configuration",
  ...
  "Configure Ollama in {highlight}opencode.json{/highlight} with {highlight}baseURL: http://localhost:11434/api{/highlight}",
  "Run {highlight}ollama list{/highlight} to see your locally available models",
  "Use {highlight}ollama pull modelname{/highlight} to download new local models",
]
